...
class CustomerRepository extends AbstractRepository implements RepositoryInterface
{
     ...
     */
    public function persist2($customer)
    {
        if ($customer->getId() === 0) {
            $mysql = "
    ...
    public function persister(object|array $data)
    {
        dump($data);
        $args = [
            'customer_name', 'owner_fname', 'owner_lname', 'street1',
    ...
    public function persist3($object)
    {
        $getters = [];
        $values = [];
